...
# ══════════════════════════════════════════════════════════════════════════════
# 🔗 HAVOLA VALIDATSIYASI — har bir xizmat turiga alohida
# ══════════════════════════════════════════════════════════════════════════════
...
LINK_RULES = {
    ...
    "tg_": {
        ...
        "hint": (
            "📌 <b>Telegram havolasi yuborish kerak</b>\n\n"
            "✅ To'g'ri formatlar:\n"
    ...
    "ig_": {
        ...
        "hint": (
            "📌 <b>Instagram havolasi yuborish kerak</b>\n\n"
            "✅ To'g'ri formatlar:\n"
    ...
    # YouTube xizmatlari
    "yt_": {
        "prefixes": ("https://www.youtube.com/", "https://youtube.com/", "https://youtu.be/", "http://youtu.be/"),
        "hint": (
            "📌 <b>YouTube havolasi yuborish kerak</b>\n\n"
            "✅ To'g'ri formatlar:\n"
    ...
    # TikTok xizmatlari
    "tt_": {
        "prefixes": ("https://www.tiktok.com/", "https://tiktok.com/", "http://tiktok.com/", "https://vm.tiktok.com/", "@"),
        "hint": (
            "📌 <b>TikTok havolasi yuborish kerak</b>\n\n"
            "✅ To'g'ri formatlar:\n"
    ...
    "tgp_": {
        "prefixes": ("@", "https://t.me/", "http://t.me/", "t.me/"),
        "hint": (
            "📌 <b>Telegram username yoki havola yuborish kerak</b>\n\n"
            "✅ To'g'ri formatlar:\n"
            "• <code>https://t.me/</code>\n"
        ),
    },
...
def get_link_rule(service_id: str) -> dict:
    """Xizmat ID'siga qarab havola qoidasini qaytaradi."""
    ...
    return {
        "prefixes": ("https://", "http://", "@", "t.me/"),
        "hint": "📌 To'g'ri havola kiriting (https://... yoki @username)"
    }
...
# ══════════════════════════════════════════════════════════════════════════════
# 💰 NARX HISOBLASH — TO'G'RILANGAN
# ══════════════════════════════════════════════════════════════════════════════

def price_label(rate_val: float) -> str:
    """
    Xizmat ro'yxatidagi narx labeli.
    rate — bu $USD/1000 dona.
    1000 ta uchun so'm = rate * USD_TO_SOM
    """
    if rate_val == 0:
        return "🆓 BEPUL"
    som_per_1000 = rate_val * USD_TO_SOM          # masalan 0.80 * 12700 = 10,160 so'm
    return f"{som_per_1000:,.0f} so'm"


def calc_total(rate_val: float, qty: int) -> float:
    """
    Buyurtma uchun umumiy summa.
    rate * USD_TO_SOM / 1000 * qty
    """
    if rate_val == 0:
        return 0.0
...
def kb_services(sub_id: str, page: int = 0):
    ...
    for svc in services[start:end]:
        ...
        builder.row(InlineKeyboardButton(
            text=f"{svc['name']} — {price_label(rate_val)}",
...
def build_service_card(svc: dict) -> str:
    ...
    if is_free:
        ...
        som_per_1000 = rate_val * USD_TO_SOM
        price_line = f"💵 Narx (x1000): <b>{som_per_1000:,.0f} so'm</b>"
...
async def order_start(callback: CallbackQuery, state: FSMContext):
    ...
    # Havola hint matnini oldindan saqlaymiz
    link_rule = get_link_rule(svc_id)
    await state.update_data(link_hint=link_rule["hint"])
...
async def process_quantity(message: Message, state: FSMContext):
    ...
    # Xizmat turiga mos hint ko'rsatamiz
...
async def confirm_order(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"⏳ Buyurtmangiz admin tomonidan ko'rib chiqilmoqda.\n"
        f"Tasdiqlangandan so'ng xizmat boshlanadi!",
        parse_mode="HTML",
    )
    await callback.answer("✅ Buyurtma adminga yuborildi!")
    ...
    admin_text = (
        ...
        f"🔑 Xizmat ID: <code>{data['service_id']}</code>\n"
        f"🔗 Havola: <code>{data['link']}</code>\n"
        f"📊 Miqdor: <b>{quantity:,} ta</b>\n"
        f"💰 Narx: <b>{price_text}</b>\n\n"
        f"✅ Tasdiqlash yoki ❌ rad etish tugmasini bosing:"
    )
...
# ══════════════════════════════════════════════════════════════════════════════
# 🛡 ADMIN TASDIQLASH HANDLERLARI
# ══════════════════════════════════════════════════════════════════════════════

@router.callback_query(F.data.startswith("admin_approve|"))
async def admin_approve_order(callback: CallbackQuery):
    """Admin buyurtmani tasdiqlaydi — SMM API ga yuboriladi."""
    ...
    # Buyurtma ma'lumotlarini bazadan olish
    ...
    # SMM API ga yuborish
    smm_order_id = await _place_smm_order(
    ...
    await callback.answer("✅ Buyurtma tasdiqlandi va SMM API ga yuborildi!")
...
async def admin_reject_order(callback: CallbackQuery):
    """Admin buyurtmani rad etadi — pul qaytariladi."""
    if callback.from_user.id != settings.ADMIN_ID:
        await callback.answer("❌ Sizda ruxsat yo'q!", show_alert=True)
        return
    ...
    # Pul qaytarish (bepul buyurtmalar uchun qaytarilmaydi)
    ...
    if charge > 0:
        ...
        await log_transaction(
            ...
            f"Qaytarish — Buyurtma #{order_id} rad etildi"
        )
...
async def _get_order_from_db(order_id: int) -> dict | None:
    """
    Bazadan buyurtma ma'lumotlarini olish.
    Bu funksiyani o'zingizning db_queries.py ga mos ravishda moslashtiring.
    """
    try:
        ...
        # Agar get_order_by_id funksiyasi yo'q bo'lsa,
        # db_queries.py ga qo'shib oling (pastdagi izoh qarang).
        logger.error("get_order_by_id funksiyasi topilmadi! db_queries.py ga qo'shing.")
        return None
    except Exception as e:
